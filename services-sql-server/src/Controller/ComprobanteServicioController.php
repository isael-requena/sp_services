...
class ComprobanteServicioController extends AbstractController{
    ...
    public function getComprobantesServicio($alcance, $entidad): Response{
        ...
        ,lr.NRO_LISTA
        ,FORMAT(lr.HORA_SALIDA, 'dd-MM-yyyy hh:mm:ss') as HORA_SALIDA
        ,FORMAT(lr.HORA_LLEGADA, 'dd-MM-yyyy hh:mm:ss') as HORA_LLEGADA
        ,lr.FEC_LISTA
        ...
        ,cs.COD_CS
        ,ISNULL(cs.CANT_ENVASES, 0) as CANT_ENVASES
        ,ISNULL(cs.DICE_CONTENER, 0) as DICE_CONTENER
        ,cs.COD_CLIENTE
        ,cs.COD_ORIGEN
        ,cs.COD_DESTINO
        ...
        ,region.ID_REGION
        ,region.NB_REGION
        --,ISNULL(tracking_cs.FK_ESTATUS, 'Pendiente') as ESTATUS_CS
        --,ISNULL(tracking_punto.FK_ESTATUS, 'Pendiente') as ESTATUS_PUNTO
        ...
        $sql .= " AND lr.FEC_LISTA = CONVERT(DATE, CURRENT_TIMESTAMP) ";
        if($alcance==3){
